...
**核心功能完善**：
- ✅ **模型列表按厂商过滤** - 从厂商卡片点击"查看模型"只显示该厂商的模型
- ✅ **接入类型列** - 模型列表新增"接入类型"列，清晰区分直连原厂/代理商
- ✅ **完整的添加厂商功能** - 带表单验证、动态字段显示、成功提示
- ✅ **统计分析Tab结构** - 改为"成本统计"和"调用日志"两个Tab
- ✅ **按业务场景拆解统计** - 新增场景维度的调用量、费用、响应时间统计
- ✅ **场景路由页面重构** - 卡片式展示各场景当前使用的模型配置
- ✅ **真正的模型切换** - 切换后自动刷新显示最新配置
- ✅ **导航优化** - 移除独立的"调用日志"菜单，整合到统计分析

**新增后端API**：
- `GET /api/active-models/all` - 获取所有场景的激活模型配置
- `GET /api/stats/by-scenario?days=30` - 按场景统计调用数据
